...
- Otherwise, select **Add API key** and fill in the _Key name_, _Role_
  and _Time to live_. Click **Add** and save the new API key.

  :::tip
  _Role_ must be either _Editor_ or _Admin_.
  :::
...
- Select the **Dashboard settings** icon, then select **JSON Model**.
  Save the dashboard _UID_ from the _JSON Model_ page
  (`GRAFANA_DASHBOARD_UID`).
...
The change will be visible in the _Query_ panel:
...
If necessary, the _Dashboard changelog_ page can be used to revert a
